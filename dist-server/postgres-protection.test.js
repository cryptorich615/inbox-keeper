...
describe('Postgres legacy protection version contract', () => {
    ...
    it('reads legacy v5, replaces to v6, rejects stale v5, adds to v7, and retains an empty-set version', async () => {
        let rules = [{ kind: 'sender', value: 'legacy@example.com', version: 5 }], state = null;
        const query = async (sql, params = []) => {
            if (sql.startsWith('SELECT kind,value,version'))
                return { rows: rules };
            if (sql.startsWith('SELECT version FROM protection_versions'))
                return { rows: state === null ? [] : [{ version: state }] };
            if (sql.startsWith('SELECT pg_advisory'))
                return { rows: [] };
            if (sql === PROTECTION_VERSION_BACKFILL_SQL) {
                const legacy = Math.max(0, ...rules.map(r => r.version));
                state = Math.max(state ?? 0, legacy);
                return { rows: [] };
            }
            if (sql.startsWith('DELETE FROM protections')) {
                rules = [];
                return { rows: [] };
            }
            if (sql.startsWith('INSERT INTO protections')) {
                rules.push({ kind: String(params[1]), value: String(params[2]), version: Number(params[3]) });
                return { rows: [] };
            }
            if (sql.startsWith('UPDATE protection_versions')) {
                state = Number(params[1]);
                return { rows: [] };
            }
            throw new Error(`Unexpected SQL: ${sql}`);
        };
        const db = { pool: { query }, transaction: async (fn) => fn({ query }) };
        const repo = new PostgresProtectionRepository(db);
