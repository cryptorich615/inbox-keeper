...
export class PostgresDatabase {
    ...
    async migrate() { const client = await this.pool.connect(); try {
...
export class PostgresTokenRepository {
    ...
    async put(userId, ciphertext, keyId, scopes, expiresAt) { await this.db.pool.query(`INSERT INTO token_envelopes(user_id,ciphertext,key_id,scopes,expires_at) VALUES($1,$2,$3,$4,$5) ON CONFLICT(user_id) DO UPDATE SET ciphertext=EXCLUDED.ciphertext,key_id=EXCLUDED.key_id,scopes=EXCLUDED.scopes,expires_at=EXCLUDED.expires_at,updated_at=now()`, [userId, ciphertext, keyId, scopes, expiresAt ? new Date(expiresAt) : null]); }
    async get(userId) { const { rows } = await this.db.pool.query('SELECT ciphertext,key_id AS "keyId",scopes,expires_at AS "expiresAt" FROM token_envelopes WHERE user_id=$1', [userId]); return rows[0] ?? null; }
    async delete(userId) { await this.db.pool.query('DELETE FROM token_envelopes WHERE user_id=$1', [userId]); }
}
export class PostgresProtectionRepository {
    ...
    async get(userId) { const [{ rows }, { rows: states }] = await Promise.all([this.db.pool.query('SELECT kind,value,version FROM protections WHERE user_id=$1', [userId]), this.db.pool.query('SELECT version FROM protection_versions WHERE user_id=$1', [userId])]); return { senders: new Set(rows.filter(r => r.kind === 'sender').map(r => r.value)), domains: new Set(rows.filter(r => r.kind === 'domain').map(r => r.value)), version: states[0] ? Number(states[0].version) : Math.max(0, ...rows.map(r => Number(r.version))) }; }
    async replace(userId, senders, domains, expectedVersion) { return this.db.transaction(async (c) => { await c.query('SELECT pg_advisory_xact_lock(hashtext($1))', [userId]); await c.query(PROTECTION_VERSION_BACKFILL_SQL, [userId]); const locked = await c.query('SELECT version FROM protection_versions WHERE user_id=$1', [userId]), current = Number(locked.rows[0].version); if (current !== expectedVersion)
        return null; const version = current + 1; await c.query('DELETE FROM protections WHERE user_id=$1', [userId]); for (const [kind, values] of [['sender', senders], ['domain', domains]])
        for (const value of values)
            await c.query('INSERT INTO protections(user_id,kind,value,version) VALUES($1,$2,$3,$4)', [userId, kind, value.toLowerCase(), version]); await c.query('UPDATE protection_versions SET version=$2 WHERE user_id=$1', [userId, version]); return version; }); }
    async add(userId, senders, domains = []) { return this.db.transaction(async (c) => { await c.query('SELECT pg_advisory_xact_lock(hashtext($1))', [userId]); await c.query(PROTECTION_VERSION_BACKFILL_SQL, [userId]); const locked = await c.query('SELECT version FROM protection_versions WHERE user_id=$1', [userId]), version = Number(locked.rows[0].version) + 1; for (const [kind, values] of [['sender', senders], ['domain', domains]])
        for (const value of values)
            await c.query('INSERT INTO protections(user_id,kind,value,version) VALUES($1,$2,$3,$4) ON CONFLICT(user_id,kind,value) DO UPDATE SET version=EXCLUDED.version', [userId, kind, value.toLowerCase(), version]); await c.query('UPDATE protection_versions SET version=$2 WHERE user_id=$1', [userId, version]); return version; }); }
}
