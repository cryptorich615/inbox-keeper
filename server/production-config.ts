...
export function loadProductionConfig(
  ...
  if (
    localTest ? env.NODE_ENV !== "development" : env.NODE_ENV !== "production"
  )
    throw new Error(
      localTest
  ...
  if (
    ...
    throw new Error(
      "OAuth callback must exactly match APP_ORIGIN + /api/oauth/callback",
    );
  ...
  return {
    ...
    googleClientId: required(
      env,
      localTest ? "GMAIL_CLIENT_ID_FILE" : "GMAIL_CLIENT_ID",
    ),
    googleClientSecretResource: required(
      env,
      localTest ? "GMAIL_CLIENT_SECRET_FILE" : "GMAIL_CLIENT_SECRET_RESOURCE",
    ),
    googleRedirectUri,
    kmsKeyName: required(
      env,
      localTest ? "LOCAL_ENVELOPE_KEY_FILE" : "GCP_KMS_KEY_NAME",
    ),
