...
export function computeInsights(
  ...
  for (const m of messages) {
    ...
    if (cat.category === 'newsletters' || cat.category === 'promotions') subscriptionCount += 1;
    if (cat.category === 'receipts') receiptCount += 1;
    if (m.hasAttachment) {
      attachmentCount += 1;
...
function signalsFromMessage(m: NormalizedMessage) {
  return {
    subject: String(m.subject ?? ""),
    snippet: String(m.subject ?? ""),
    fromAddress: String(m.fromAddress ?? ""),
