...
const db: BrandDb = JSON.parse(readFileSync(dbPath, 'utf8'));

const BRAND_BY_DOMAIN = new Map<string, BrandDbEntry>();
...
function stripTag(local: string): string {
  ...
  if (plus >= 0) return local.slice(0, plus);
  return local;
}

export function extractRootDomain(address: string): string {
...
function lookupBrand(rootDomain: string): BrandDbEntry | undefined {
...
function prettifyRootDomain(rootDomain: string): string {
  const sld = rootDomain.split('.')[0] ?? rootDomain;
  if (!sld) return rootDomain;
...
export function extractBrandKey(rawSender: string): ExtractedBrand {
