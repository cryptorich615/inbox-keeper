...
import type { StoredGoogleTokens } from "./token-service.js";
...
class ProtectedSenderSet extends Set<string> {
  override has(value: string) {
    return !/^[^@\s<>]+@[^@\s<>]+$/.test(value) || super.has(value);
  }
}
const cookie = (id: string, local = false) =>
  `${local ? "inbox_local_session" : "__Host-inbox_session"}=${encodeURIComponent(id)}; Path=/; HttpOnly; ${local ? "" : "Secure; "}SameSite=Lax; Max-Age=1800`;
...
const sessionId = (req: IncomingMessage) =>
  /\b(?:__Host-inbox_session|inbox_local_session)=([^;]+)/.exec(
    String(req.headers.cookie || ""),
  )?.[1];

async function main() {
  const cfg = loadProductionConfig(),
    clientSecret = cfg.localTest
      ? await readLocalSecret(cfg.googleClientSecretResource)
      : await accessSecret(cfg.googleClientSecretResource),
    clientId = cfg.localTest
      ? await readLocalSecret(cfg.googleClientId)
      : cfg.googleClientId,
    db = new PostgresDatabase(cfg.databaseUrl);
  await db.migrate();
  const kms = cfg.localTest
      ? await LocalFileKms.fromFile(cfg.kmsKeyName)
      : new GoogleCloudKms(cfg.kmsKeyName),
    cipher = new TokenEnvelopeCipher(kms),
    tokens = new PostgresTokenRepository(db),
    oauth = new GoogleOAuthHttp({
      clientId,
      clientSecret,
  ...
  const taskAuth = cfg.localTest
      ? null
      : new GoogleOidcVerifier(cfg.taskAudience, cfg.taskServiceAccount),
    dist = resolve("dist");
  let ready = true;
  const getSession = async (req: IncomingMessage) => {
    ...
    const { rows } = await db.pool.query(
      'SELECT id_hash,user_id AS "userId",csrf_hash,expires_at FROM sessions WHERE id_hash=$1 AND expires_at>now()',
      [sha(decodeURIComponent(raw))],
    );
    return rows[0] ? { ...rows[0], raw } : null;
  };
  const requireMutation = (req: IncomingMessage, s: { csrf_hash: string }) => {
  ...
  const access = async (userId: string) =>
    db.transaction(async (client) => {
      ...
      const { rows } = await client.query(
        "SELECT ciphertext FROM token_envelopes WHERE user_id=$1 FOR UPDATE",
        [userId],
      );
      if (!rows[0]) throw new Error("Gmail not connected");
      const value = await cipher.decrypt<StoredGoogleTokens>(
        rows[0].ciphertext,
      );
      if (value.expiresAt > Date.now() + 60_000) return value.accessToken;
      const refreshed = await oauth.refresh(value.refreshToken),
        updated = {
          ...value,
          ...refreshed,
      ...
      await client.query(
        "UPDATE token_envelopes SET ciphertext=$1,key_id=$2,scopes=$3,expires_at=$4,updated_at=now() WHERE user_id=$5",
        [
          sealed,
          cipher.keyId,
          updated.scopes,
  ...
  const protections = async (userId: string) => {
    const [{ rows },state] = await Promise.all([db.pool.query(
      "SELECT kind,value,version FROM protections WHERE user_id=$1",
      [userId],
    ),db.pool.query("SELECT version FROM protection_versions WHERE user_id=$1",[userId])]);
    return {
      senders: new ProtectedSenderSet(
        rows
  ...
  const isProtected = (
    addressInput: string,
    rules: Awaited<ReturnType<typeof protections>>,
  ) => {
    const address = addressInput.trim().toLowerCase();
    if (!/^[^@\s<>]+@[^@\s<>]+$/.test(address)) return true;
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === "POST" && req.url === "/internal/tasks/reconcile") {
        ...
        const { rows } = await db.pool.query(
          `SELECT ji.user_id AS "userId",ji.job_id AS "jobId",ji.message_id AS "messageId",j.action FROM job_items ji JOIN jobs j ON j.user_id=ji.user_id AND j.id=ji.job_id WHERE ji.status='unknown' ORDER BY ji.updated_at LIMIT 50`,
        );
        for (const item of rows) {
          try {
            const provider = new GmailHttpProvider(() => access(item.userId)),
              [message] = await provider.currentMetadata([item.messageId]),
              trashed = message.labels.includes("TRASH"),
              confirmed = item.action === "trash" ? trashed : !trashed;
            await db.pool.query(
              `UPDATE job_items SET status=$1,reason=$2,updated_at=now() WHERE user_id=$3 AND job_id=$4 AND message_id=$5`,
              [
      ...
      if (req.method === "POST" && req.url === "/api/session") {
        ...
        await db.pool.query(
          "INSERT INTO sessions(id_hash,user_id,csrf_hash,expires_at) VALUES($1,NULL,$2,now()+interval '30 minutes')",
          [sha(raw), sha(csrf)],
        );
      ...
      if (req.method === "GET" && req.url === "/api/status") {
        ...
        await db.pool.query(
          "UPDATE sessions SET csrf_hash=$1 WHERE id_hash=$2",
          [sha(csrf), session.id_hash],
        );
        if (!session.userId)
          return json(res, 200, {
            mode: "disconnected",
        ...
        const token = await tokens.get(session.userId),
          { rows } = await db.pool.query(
            "SELECT email FROM users WHERE id=$1",
            [session.userId],
          ),
          scopes = token?.scopes ?? [],
          mode = scopes.includes("https://www.googleapis.com/auth/gmail.modify")
            ? "cleanup"
            : "readonly";
        return json(res, 200, {
          mode,
      ...
      if (req.method === "POST" && req.url === "/api/oauth/start") {
        ...
        const input = await body(req),
          pair = pkce(),
          state = opaqueState(),
          sealed = await cipher.encrypt({ verifier: pair.verifier, state });
        await db.pool.query(
          "UPDATE sessions SET oauth_state_hash=$1,pkce_ciphertext=$2 WHERE id_hash=$3",
          [sha(state), sealed, session.id_hash],
        );
        return json(res, 200, {
      ...
      if (req.method === "GET" && isOAuthCallback(req.url)) {
        const u = new URL(req.url || "/api/oauth/callback", cfg.appOrigin),
          state = u.searchParams.get("state") || "",
          code = u.searchParams.get("code") || "";
        const { rows } = await db.pool.query(
          "DELETE FROM sessions WHERE id_hash=$1 AND oauth_state_hash=$2 AND expires_at>now() RETURNING pkce_ciphertext",
          [session.id_hash, sha(state)],
        );
        if (!rows[0] || !code)
          throw Object.assign(new Error("OAuth callback rejected"), {
            status: 400,
          });
        const saved = await cipher.decrypt<{ verifier: string; state: string }>(
            rows[0].pkce_ciphertext,
          ),
          grant = await oauth.exchangeCode({
            code,
        ...
        await db.transaction(async (c) => {
          await c.query(
            "INSERT INTO users(id,google_subject,email) VALUES($1,$2,$3) ON CONFLICT(google_subject) DO UPDATE SET email=EXCLUDED.email",
            [userId, grant.identity.sub, grant.identity.email ?? null],
          );
          await c.query(
            `INSERT INTO token_envelopes(user_id,ciphertext,key_id,scopes,expires_at) VALUES($1,$2,$3,$4,$5) ON CONFLICT(user_id) DO UPDATE SET ciphertext=EXCLUDED.ciphertext,key_id=EXCLUDED.key_id,scopes=EXCLUDED.scopes,expires_at=EXCLUDED.expires_at,updated_at=now()`,
            [
              userId,
              sealed,
              cipher.keyId,
          ...
          const raw = randomBytes(32).toString("base64url"),
            csrf = randomBytes(24).toString("base64url");
          await c.query(
            "INSERT INTO sessions(id_hash,user_id,csrf_hash,expires_at) VALUES($1,$2,$3,now()+interval '30 minutes')",
            [sha(raw), userId, sha(csrf)],
          );
          res.setHeader("set-cookie", cookie(raw, cfg.localTest));
      ...
      if (req.method === "POST" && req.url === "/api/sync") {
        ...
        for (const message of page.items)
          await db.pool.query(
            "INSERT INTO message_cache(user_id,message_id,metadata) VALUES($1,$2,$3) ON CONFLICT(user_id,message_id) DO UPDATE SET metadata=EXCLUDED.metadata,updated_at=now()",
            [session.userId, message.id, message],
          );
        return json(res, 200, {
          messages: page.items,
      ...
      if (req.method === "GET" && req.url === "/api/messages") {
        const { rows } = await db.pool.query(
          "SELECT metadata FROM message_cache WHERE user_id=$1 ORDER BY updated_at DESC",
          [session.userId],
        );
      ...
      if (req.method === "GET" && req.url === "/api/brands") {
        ...
        const { rows: messages } = await db.pool.query(
          "SELECT metadata FROM message_cache WHERE user_id=$1",
          [session.userId],
        );
        const { rows: overrides } = await db.pool.query(
          "SELECT brand_key, display_name, merged_keys FROM user_brand_overrides",
        );
        const overrideMap = new Map<string, { displayName: string; mergedKeys: string[] }>();
        ...
        for (const row of messages) {
          ...
          if (sender && bucket.sampleSenders.size < 5) bucket.sampleSenders.add(sender);
          const received = String(m.receivedAt ?? "");
          if (received && received > bucket.lastMessageAt) bucket.lastMessageAt = received;
      ...
      if (req.method === "GET" && req.url?.startsWith("/api/brands/") && !req.url.endsWith("/rename")) {
        ...
        const { rows: overrides } = await db.pool.query(
          "SELECT brand_key, display_name FROM user_brand_overrides WHERE brand_key=$1",
          [brandKey],
        );
        const override = overrides[0];
        const categories: Record<EmailCategory, NormalizedMessage[]> = {
        ...
        for (const row of messages) {
          ...
          if (ext.brandKey !== brandKey) continue;
          if (!override) displayName = ext.displayName;
      ...
      if (req.method === "POST" && req.url?.startsWith("/api/brands/") && req.url.endsWith("/rename")) {
        ...
        await db.pool.query(
          "INSERT INTO user_brand_overrides(brand_key, display_name) VALUES($1,$2) ON CONFLICT(brand_key) DO UPDATE SET display_name=EXCLUDED.display_name, updated_at=now()",
          [brandKey, newName],
        );
      ...
      if (req.method === "GET" && req.url === "/api/activity") {
        if (!session.userId)
          throw Object.assign(new Error("Gmail not connected"), { status: 409 });
        const { rows } = await db.pool.query(
          "SELECT id,event_type AS \"eventType\",event,created_at AS \"createdAt\" FROM audit WHERE user_id=$1 ORDER BY created_at DESC LIMIT 500",
          [session.userId],
        );
      ...
      if (req.method === "PUT" && req.url === "/api/protections") {
        ...
        const clean = (v: unknown) => String(v).trim().toLowerCase(),
          ...
          domains = [
            ...new Set(
              input.domains
                .map(clean)
                .filter((v: string) => /^(?:[a-z0-9-]+\.)+[a-z]{2,}$/i.test(v)),
            ),
          ];
        const version = await db.transaction(async (c) => {
          ...
          const current = await c.query(
              "SELECT version FROM protection_versions WHERE user_id=$1",
              [session.userId],
            ),
            currentVersion = current.rows[0]?Number(current.rows[0].version):0;
          ...
          for (const value of senders)
            await c.query(
              "INSERT INTO protections(user_id,kind,value,version) VALUES($1,'sender',$2,$3)",
              [session.userId, value, next],
            );
          for (const value of domains)
            await c.query(
              "INSERT INTO protections(user_id,kind,value,version) VALUES($1,'domain',$2,$3)",
              [session.userId, value, next],
            );
        ...
        if(version===null) throw Object.assign(new Error("Protected mail changed in another session. Latest rules were reloaded; review and retry your change."),{status:409});
      ...
      if (req.method === "POST" && req.url === "/api/protections/senders") {
        ...
        const senders = [...new Set<string>(input.senders.map((v:unknown)=>String(v).trim().toLowerCase()).filter((v:string)=>/^[^@\s]+@[^@\s]+$/.test(v)))];
        if (!senders.length) throw Object.assign(new Error("No valid sender addresses"), { status: 400 });
        const version = await db.transaction(async c=>{
          ...
          for(const value of senders) await c.query("INSERT INTO protections(user_id,kind,value,version) VALUES($1,'sender',$2,$3) ON CONFLICT(user_id,kind,value) DO UPDATE SET version=EXCLUDED.version",[session.userId,value,next]);
          await c.query("INSERT INTO protection_versions(user_id,version) VALUES($1,$2) ON CONFLICT(user_id) DO UPDATE SET version=EXCLUDED.version",[session.userId,next]);
          return next;
        });
        const rules=await protections(session.userId);
      ...
      if (req.method === "POST" && req.url === "/api/previews") {
        ...
        const input = await body(req),
          ids: string[] = Array.isArray(input.ids)
            ? [
                ...new Set<string>(input.ids.map((v: unknown) => String(v))),
              ].slice(0, 1000)
            : [];
        if (!ids.length || !["trash", "restore"].includes(String(input.action)))
          throw Object.assign(new Error("Invalid preview"), { status: 400 });
        const action: "trash" | "restore" =
            input.action === "restore" ? "restore" : "trash",
          provider = new GmailHttpProvider(() => access(session.userId)),
          messages = await provider.currentMetadata(ids),
          rules = await protections(session.userId),
          found = new Set(messages.map((m) => m.id)),
          excluded = messages
            .filter((m) => isProtected(m.fromAddress, rules))
            .map((m) => m.id),
          missing = ids.filter((id) => !found.has(id)),
          included = ids.filter((id) => found.has(id) && !excluded.includes(id)),
          previewId = randomUUID(),
          confirmText = `${action === "trash" ? "MOVE" : "RESTORE"} ${included.length}`;
        await db.pool.query(
          `INSERT INTO previews(id,session_hash,user_id,action,ids,excluded,rule_version,confirm_text,expires_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,now()+interval '10 minutes')`,
          [
            previewId,
            session.id_hash,
            session.userId,
      ...
      if (req.method === "POST" && req.url === "/api/jobs") {
        ...
        const prior = await db.pool.query(
          "SELECT result FROM jobs WHERE user_id=$1 AND id=$2",
          [session.userId, idempotency],
        );
        if (prior.rows[0]?.result) return json(res, 200, prior.rows[0].result);
        const input = await body(req),
          claimed = await db.pool.query(
            `UPDATE previews SET claimed_at=now() WHERE id=$1 AND session_hash=$2 AND user_id=$3 AND claimed_at IS NULL AND expires_at>now() AND (confirm_text IS NULL OR confirm_text=$4) RETURNING action,ids`,
            [
        ...
        if (!claimed.rows[0])
          throw Object.assign(
            new Error("Preview expired, used, or confirmation rejected"),
            { status: 409 },
          );
        const action = claimed.rows[0].action as "trash" | "restore",
          ids = claimed.rows[0].ids as string[],
          provider = new GmailHttpProvider(() => access(session.userId)),
          items = [] as Array<{ id: string; status: string; reason?: string }>;
        await db.pool.query(
          "INSERT INTO jobs(id,user_id,action,status) VALUES($1,$2,$3,'running') ON CONFLICT DO NOTHING",
          [idempotency, session.userId, action],
        );
        for (const id of ids) {
          try {
            const [message] = await provider.currentMetadata([id]),
              rules = await protections(session.userId),
              address = message.fromAddress.toLowerCase(),
          ...
          await db.pool.query(
            "INSERT INTO job_items(user_id,job_id,message_id,status,reason) VALUES($1,$2,$3,$4,$5) ON CONFLICT(user_id,job_id,message_id) DO UPDATE SET status=EXCLUDED.status,reason=EXCLUDED.reason,updated_at=now()",
            [
              session.userId,
              idempotency,
              last.id,
        ...
        await db.transaction(async (c) => {
          await c.query(
            "UPDATE jobs SET status=$1,result=$2,updated_at=now() WHERE user_id=$3 AND id=$4",
            [
          ...
          await c.query(
            "INSERT INTO audit(id,user_id,event_type,event) VALUES($1,$2,'cleanup_job',$3)",
            [randomUUID(), session.userId, result],
          );
        });
      ...
      if (req.method === "POST" && req.url === "/api/disconnect") {
        ...
        res.setHeader(
          "set-cookie",
          cookie("", cfg.localTest).replace("Max-Age=1800", "Max-Age=0"),
        );
        return json(res, 200, { ok: true });
      }
      ...
      if (req.method === "GET" && (await serve(req, res))) return;
      throw Object.assign(new Error("Not found"), { status: 404 });
    } catch (error) {
      const status =
      ...
      json(res, status, {
        error:
          status >= 500 ? "Request failed safely" : (error as Error).message,
        requestId,
      });
    }
  });
